...
**Environment** — `env.ts` reads from `window.env` (production, injected by `env.sh` script) or `import.meta.env` (dev). All env vars use `VITE_SHELLHUB_` prefix.
...
| `text-muted` | `#5C6070` | Disabled/muted text |
| `primary` | `#667ACC` | Primary actions, links |
| `accent-green` | `#82a568` | Online, success |
| `accent-red` | `#ca6169` | Errors, destructive |
