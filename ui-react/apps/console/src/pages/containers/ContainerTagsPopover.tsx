...
function ContainerTagsPopover({
  ...
  const addTag = useAddContainerTag();
  const removeTag = useRemoveContainerTag();
  ...
  const handleAdd = async (tag: string) => {
    ...
    setError(null);
    try {
      await addTag.mutateAsync({ path: { uid: container.uid, name: tag } });
      setInput("");
    } catch (e) {
  ...
  const handleRemove = async (tag: string) => {
    ...
    setError(null);
    try {
      await removeTag.mutateAsync({ path: { uid: container.uid, name: tag } });
    } catch (e) {
      const status = isSdkError(e) ? e.status : undefined;
      ...
      <div className="flex items-center gap-1 min-h-[28px] group/tags">
        ...
        {canEditTags && (
          <button
            ...
            title="Manage tags"
            aria-label="Manage container tags"
          >
...
export default ContainerTagsPopover;
