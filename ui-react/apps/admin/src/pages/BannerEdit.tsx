    ...
    <div className="flex flex-col flex-1 min-h-0 animate-fade-in">
      ...
      >
        <textarea
          ...
          className="flex-1 w-full px-4 py-3.5 bg-card text-sm text-text-primary font-mono placeholder:text-text-muted/30 focus:outline-none transition-all resize-none leading-relaxed disabled:opacity-50 disabled:cursor-not-allowed"
          autoFocus
        />
      ...
      <div className="shrink-0">
        ...
        {canEdit && (
          <div className="flex items-center justify-end gap-2 mt-6 pt-6 border-t border-border">
            <Link
              to="/settings"
              className="px-4 py-2.5 text-sm font-medium text-text-secondary hover:text-text-primary rounded-lg hover:bg-white/[0.03] transition-colors"
            >
              Cancel
            </Link>
            <button
              ...
              className="px-5 py-2.5 bg-primary hover:bg-primary/90 text-white rounded-lg text-sm font-semibold disabled:opacity-40 disabled:cursor-not-allowed transition-all flex items-center gap-2"
            >
              {saving ? (
...
export default function BannerEdit() {
  ...
  const isOwner =
    ns.owner === userId || ns.owner === localStorage.getItem("userId");
  const currentMember = ns.members?.find(
    (m) => m.id === userId || m.id === localStorage.getItem("userId"),
  );
  const role =
    currentMember?.role ??
    (isOwner ? "owner" : (localStorage.getItem("role") ?? "observer"));
    ...
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="relative -mx-8 -mt-8 px-8 py-6 mb-8 border-b border-border bg-surface shrink-0">
        <div className="flex items-start gap-4">
          <Link
            to="/settings"
            className="w-12 h-12 rounded-lg bg-white/[0.04] border border-border flex items-center justify-center text-text-muted hover:text-text-primary hover:border-border-light transition-all shrink-0"
          >
            <ArrowLeftIcon className="w-5 h-5" />
          </Link>
          <div>
            <p className="text-2xs font-mono font-semibold uppercase tracking-[0.15em] text-primary mb-1">
              Settings
            </p>
