...
import { type ApiKey } from "../../api/team";
import ConfirmDialog from "../../components/common/ConfirmDialog";
import { RoleBadge, isExpired } from "./constants";
    ...
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-5">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-primary hover:bg-primary/90 text-white rounded-lg text-sm font-semibold transition-all"
        >
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left px-4 py-3 text-2xs font-mono font-semibold uppercase tracking-[0.15em] text-text-muted">
                  Name
                </th>
                <th className="text-left px-4 py-3 text-2xs font-mono font-semibold uppercase tracking-[0.15em] text-text-muted">
                  Role
                ...
                <th className="text-right px-4 py-3 text-2xs font-mono font-semibold uppercase tracking-[0.15em] text-text-muted w-24">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {apiKeys.map((key) => {
                ...
                return (
                  <tr
                    key={key.id}
                    className={`group transition-colors ${expired ? "bg-accent-red/[0.02]" : "hover:bg-white/[0.02]"}`}
                  >
            ...
            <div className="flex items-center justify-between px-4 py-3 border-t border-border">
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="p-1.5 rounded-md text-text-muted hover:text-text-primary hover:bg-white/[0.05] disabled:opacity-30 transition-colors"
                >
