  ...
  return (
    <>
      ...
      >
        ...
        Tags
        {hasActive && (
          <span className="w-4 h-4 rounded-full bg-primary text-white text-[9px] font-bold flex items-center justify-center leading-none">
            {filterTags.length}
          </span>
        )}
        <ChevronDownIcon
      ...
      {open &&
        createPortal(
          <div
            ref={popoverRef}
            className="fixed z-[9999] w-[240px] bg-surface border border-border rounded-xl shadow-2xl animate-fade-in"
                ...
                filtered.map((tag) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full flex items-center gap-2 px-2.5 py-1.5 text-xs rounded-md hover:bg-white/[0.05] transition-colors"
                    >
                      <span
