...
describe("MfaEnableDrawer", () => {
  ...
  describe("Step 1: Recovery Email", () => {
    ...
    it("shows error when email is already in use (409)", async () => {
      ...
      mockedUpdateUser.mockRejectedValue(
        Object.assign(new Error("Email conflict"), { isAxiosError: true, response: { status: 409 } })
      );
  ...
  describe("State Cleanup", () => {
    it("resets state when drawer is closed and reopened", async () => {
      ...
      // Should be back at step 1 with the existing recovery email shown
      // Email may be in a truncated element; use getByText with exact: false
      expect(screen.getByText(/recovery@example\.com/, { exact: false })).toBeInTheDocument();
    });
  });
