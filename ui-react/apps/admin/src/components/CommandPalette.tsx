...
const badgeStyles: Record<BadgeVariant, string> = {
  ...
  muted: "text-text-muted bg-white/[0.04] border-border",
};
    ...
    <div className="fixed inset-0 z-[80] flex items-start justify-center pt-[min(20vh,140px)]">
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 border-b border-border">
          ...
          />
          <kbd className="shrink-0 px-1.5 py-0.5 text-[10px] font-mono font-semibold text-text-muted/50 bg-white/[0.04] border border-border rounded">
            ESC
          </kbd>
        </div>
        ...
        >
          {query.toLowerCase().trim() === "gustavo" ? (
            <div className="px-4 py-6 flex flex-col items-center gap-3 animate-fade-in">
              <img
                src={gustavoImg}
                alt="Gustavo"
                className="w-32 h-32 rounded-full object-cover border-2 border-primary/30 shadow-lg shadow-primary/10"
              />
            ...
            Array.from(sections.entries()).map(([section, sectionItems]) => (
              <div key={section}>
                <div className="px-4 pt-3 pb-1.5">
                  <p className="text-2xs font-mono font-semibold uppercase tracking-[0.15em] text-text-muted/50">
                    {section}
                  </p>
                </div>
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-2.5 text-left transition-colors duration-75 ${
                        isActive ? "bg-primary/[0.08]" : "hover:bg-white/[0.02]"
                      }`}
                    >
                      <span
                      ...
                      {isActive && (
                        <kbd className="shrink-0 px-1.5 py-0.5 text-[10px] font-mono text-text-muted/40 bg-white/[0.03] border border-border/50 rounded">
                          ↵
                        </kbd>
                      )}
                    </button>
        ...
        <div className="flex items-center gap-4 px-4 py-2.5 border-t border-border bg-card/30">
          <div className="flex items-center gap-1.5">
            <kbd className="px-1 py-0.5 text-[10px] font-mono text-text-muted/40 bg-white/[0.03] border border-border/50 rounded">
              ↑
            </kbd>
            <kbd className="px-1 py-0.5 text-[10px] font-mono text-text-muted/40 bg-white/[0.03] border border-border/50 rounded">
              ↓
          ...
          <div className="flex items-center gap-1.5 ml-auto">
            <kbd className="px-1.5 py-0.5 text-[10px] font-mono text-text-muted/40 bg-white/[0.03] border border-border/50 rounded">
              ⌘K
            </kbd>
