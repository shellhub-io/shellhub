...
function NavItemLink({ item }: { item: NavItem }) {
  ...
  const showBadge = item.premium && !env.isCloud && !env.isEnterprise;
    ...
    >
      ...
      <div className="px-3 pt-4 pb-2">
        <button
          ...
          className="flex items-center gap-2.5 w-full px-3 py-2 rounded-md border border-border hover:border-border-light bg-white/[0.02] hover:bg-white/[0.04] transition-all duration-150 group"
        >
          ...
          <kbd className="text-[10px] font-mono text-text-muted/30 bg-white/[0.03] border border-border/50 px-1.5 py-0.5 rounded shrink-0">
            ⌘K
          </kbd>
        </button>
      </div>
      ...
      <nav className="flex-1 px-3 py-2 overflow-y-auto">
        {sections.map((section, idx) => (
          <div key={section.title} className={idx > 0 ? "mt-5" : ""}>
            <p className="px-3 mb-1.5 text-2xs font-mono font-semibold uppercase tracking-[0.15em] text-text-muted/60">
              {section.title}
            </p>
