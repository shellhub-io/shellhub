  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      <button
        ...
        className="relative flex items-center justify-center w-8 h-8 rounded-lg border border-transparent hover:border-border hover:bg-white/[0.03] transition-all duration-150"
      >
        <CpuChipIcon className="w-[18px] h-[18px] text-text-secondary" />

        {/* Badge */}
        {count > 0 && (
          <span className="absolute -top-0.5 -right-0.5 min-w-[16px] h-4 px-1 flex items-center justify-center rounded-full bg-accent-yellow text-[9px] font-bold font-mono text-background leading-none">
            {count > 99 ? "99+" : count}
          </span>
        )}
      </button>
        ...
        <div className="absolute top-full right-0 mt-1.5 w-80 bg-surface border border-border rounded-lg shadow-2xl shadow-black/40 z-50 overflow-hidden animate-slide-down">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-border">
            ...
            <button
              ...
              className="p-1 rounded-md text-text-muted hover:text-text-primary hover:bg-white/[0.05] transition-colors"
            >
                        ...
                        {isFlashed ? (
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            <button
                              ...
                              disabled={isActing}
                              className="p-1.5 rounded-md text-text-muted hover:text-accent-green hover:bg-accent-green/10 transition-colors disabled:opacity-30"
                              title="Accept"
                            >
                              {isActing ? (
                            ...
                            <button
                              ...
                              disabled={isActing}
                              className="p-1.5 rounded-md text-text-muted hover:text-accent-red hover:bg-accent-red/10 transition-colors disabled:opacity-30"
                              title="Reject"
                            >
                              <XMarkIcon
                                className="w-3.5 h-3.5"
