...
describe("useKeyFileInput", () => {
  ...
  describe("paste interception", () => {
    ...
    it("validates trimmed text but passes raw text to onChange", () => {
      ...
      expect(onChange).toHaveBeenCalledWith("  ssh-rsa AAAAB  ");
    });
    ...
    it("does not intercept paste when a text input is focused", () => {
      const onChange = vi.fn();
      renderHook(() =>
        useKeyFileInput({ validate: () => true, onChange }),
      );
    ...
    it("does not intercept paste when a textarea is focused", () => {
      const onChange = vi.fn();
      renderHook(() =>
        useKeyFileInput({ validate: () => true, onChange }),
      );
