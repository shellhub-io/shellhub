...
func NewTunnel(connection string, dial string, config Config) (*Tunnel, error) {
	...
	tunnel.Tunnel.ConnectionHandler = func(request *http.Request) (string, error) {
		...
		if tenant == "" {
			device, err := tunnel.API.GetDevice(context.Background(), uid)
			if err != nil {
				log.WithError(err).
					WithField("uid", uid).
	...
	tunnel.Tunnel.CloseHandler = func(key string) {
		...
		if err := tunnel.API.DevicesOffline(context.Background(), uid); err != nil {
			log.WithError(err).
	...
	tunnel.Tunnel.KeepAliveHandler = func(key string) {
		...
		if err := tunnel.API.DevicesHeartbeat(context.Background(), uid); err != nil {
			log.WithError(err).
				WithFields(log.Fields{
					"uid":       uid,
					"tenant_id": tenant,
