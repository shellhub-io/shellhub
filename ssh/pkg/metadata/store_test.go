...
func TestStoreAuthenticationMethod(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			setup: func(ctx *gliderssh.Context) *sshsrvtest.Conn {
				return sshsrvtest.New(
					&gliderssh.Server{
						Handler: func(s gliderssh.Session) {
							*ctx = s.Context()
							StoreAuthenticationMethod(*ctx, PasswordAuthenticationMethod)
						},
					},
					&gossh.ClientConfig{
						User: "user",
			...
			expected: PasswordAuthenticationMethod,
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.description, func(t *testing.T) {
			...
			assert.NoError(t, srv.Agent.Run(""))
			assert.Equal(t, tc.expected, ctx.Value(authentication).(AuthenticationMethod))
		})
	}
}
...
func TestMaybeStoreFingerprint(t *testing.T) {
			...
			setup: func(ctx *gliderssh.Context) *sshsrvtest.Conn {
				return sshsrvtest.New(
					&gliderssh.Server{
						Handler: func(s gliderssh.Session) {
							*ctx = s.Context()
							MaybeStoreFingerprint(*ctx, "fingerprint")
						},
					},
					&gossh.ClientConfig{
						User: "user",
				...
				return sshsrvtest.New(
					&gliderssh.Server{
						Handler: func(s gliderssh.Session) {
							...
							*ctx = s.Context()
							MaybeStoreFingerprint(*ctx, "other value")
						},
					},
					&gossh.ClientConfig{
						User: "user",
	...
	for _, tc := range cases {
		t.Run(tc.description, func(t *testing.T) {
			...
			assert.NoError(t, srv.Agent.Run(""))
			assert.Equal(t, tc.expected, ctx.Value(fingerprint).(string))
		})
	}
}
...
func TestMaybeStoreEstablished(t *testing.T) {
	cases := []struct {
		...
		{
			description: "succeeds in storing established",
			setup: func(ctx *gliderssh.Context) *sshsrvtest.Conn {
				return sshsrvtest.New(
					&gliderssh.Server{
						Handler: func(s gliderssh.Session) {
		...
		{
			description: "succeeds in retrieving established when it is already defined",
			setup: func(ctx *gliderssh.Context) *sshsrvtest.Conn {
				return sshsrvtest.New(
					&gliderssh.Server{
						Handler: func(s gliderssh.Session) {
							...
							*ctx = s.Context()
							MaybeStoreEstablished(*ctx, false)
						},
					},
					&gossh.ClientConfig{
						User: "user",
