...
func (*publicKeyAuth) Auth() authFunc {
	return func(session *Session, config *gossh.ClientConfig) error {
		privateKey, err := session.api.CreatePrivateKey(context.Background())
		if err != nil {
			return err
		}
...
func (p *publicKeyAuth) Evaluate(session *Session) error {
	...
	if gossh.FingerprintLegacyMD5(magic) != fingerprint {
		if _, err = session.api.GetPublicKey(context.Background(), fingerprint, session.Device.TenantID); err != nil {
			return err
		}

		if ok, err := session.api.EvaluateKey(context.Background(), fingerprint, session.Device, session.Data.Target.Username); !ok || err != nil {
