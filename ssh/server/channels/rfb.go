...
func DefaultRFBHandler() gliderssh.ChannelHandler {
	return func(srv *gliderssh.Server, conn *gossh.ServerConn, newChan gossh.NewChannel, ctx gliderssh.Context) {
		...
		logger.Trace("accepting rfb channel")
		...
		logger.Trace("rfb channel accepted")

		d := newChan.ExtraData()

		logger.Trace("opening rfb channel on agent")

		agent, err := sess.NewAgentChannelWithData(RFBChannel, seat, d)
		if err != nil {
		...
		logger.Trace("rfb channel opened on agent")
		...
		go func() {
			defer wg.Done()
			defer logger.Trace("rfb channel data copy done")

			logger.Trace("starting rfb channel data copy from agent to client")
		...
		go func() {
			...
			logger.Trace("starting rfb channel data copy from client to agent")
