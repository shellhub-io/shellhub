...
import * as apiKeysApi from "@/store/api/api_keys";
...
describe("Team Api Keys View", () => {
  ...
  const mountWrapper = async (hasKeys = true, mockError?: Error) => {
    ...
    if (mockError) {
      ...
      vi.mocked(apiKeysApi.getApiKey).mockRejectedValueOnce(mockError);
    }
  ...
  describe("when loading API keys fails", () => {
    beforeEach(async () => {
      await mountWrapper(false, new Error("Failed to fetch API keys"));
    });
  ...
  describe("loading state", () => {
    it("shows loading spinner initially", async () => {
      vi.mocked(apiKeysApi.getApiKey).mockImplementation(
        () => new Promise((resolve) => setTimeout(() => resolve({ data: [], headers: {} } as AxiosResponse), 100)),
      );
