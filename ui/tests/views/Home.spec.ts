...
describe("Home View", () => {
  ...
  const mountWrapper = async (hasNamespace = true, deviceStats = { total: 5, online: 3, pending: 1 }) => {
  ...
  describe("when namespace exists", () => {
    ...
    it("updates displayed statistics when different values are provided", async () => {
      wrapper.unmount();
      await mountWrapper(true, { total: 10, online: 8, pending: 2 });
