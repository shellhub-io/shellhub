...
describe("Details Sessions View", () => {
  ...
  const setupTest = async ({
    ...
    if (mockError) vi.mocked(sessionsStore.getSession).mockRejectedValue(mockError);
  ...
  describe("when session loads successfully", () => {
    ...
    it.each([
      ...
      async (recorded, shouldShowButton) => {
        await setupTest({ initialSession: { ...mockSession, recorded } });
  ...
  describe("when session fails to load", () => {
    beforeEach(() => setupTest({ sessionId: "inexistent-session", mockError: createAxiosError(404, "Not Found") }));
