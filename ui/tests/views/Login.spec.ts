...
describe("Login View", () => {
  ...
  describe("user login flow", () => {
    ...
    it("preserves redirect path from query params", async () => {
      await mountWrapper({ route: "/login?redirect=/devices", stubActions: true });
  ...
  describe("alert messages", () => {
    it("displays alert when redirected from accept invite", async () => {
      await mountWrapper({
        route: "/login?redirect=/accept-invite/test-id",
        initialState: {
    ...
    it("displays alert when SSO has missing assertions", async () => {
      await mountWrapper({ route: "/login?missing_assertions=true" });
