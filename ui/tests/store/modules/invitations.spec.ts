...
describe("Invitations Store", () => {
  ...
  describe("fetchUserPendingInvitationList", () => {
    it("should fetch user pending invitations successfully", async () => {
      ...
      mockNamespacesApi
        .onGet(`http://localhost:3000/api/users/invitations?filter=${encodedPendingInvitationsFilter}&page=1&per_page=100`)
    ...
    it("should handle server error when fetching pending invitations", async () => {
      mockNamespacesApi
        .onGet(`http://localhost:3000/api/users/invitations?filter=${encodedPendingInvitationsFilter}&page=1&per_page=100`)
        .reply(500, { message: "Internal Server Error" });

      await expect(store.fetchUserPendingInvitationList()).rejects.toBeAxiosErrorWithStatus(500);
    });
  ...
  describe("fetchNamespaceInvitationList", () => {
    it("should fetch namespace invitations successfully with pagination", async () => {
      ...
      mockNamespacesApi
        .onGet("http://localhost:3000/api/namespaces/tenant-123/invitations?page=1&per_page=10")
        .reply(200, mockInvitations, {
          "x-total-count": "2",
        });
    ...
    it("should fetch namespace invitations with filter", async () => {
      ...
      mockNamespacesApi
        .onGet("http://localhost:3000/api/namespaces/tenant-123/invitations?filter=test-filter&page=1&per_page=10")
        .reply(200, mockInvitations, {
          "x-total-count": "1",
        });
    ...
    it("should handle server error when fetching namespace invitations", async () => {
      mockNamespacesApi
        .onGet("http://localhost:3000/api/namespaces/tenant-123/invitations?page=1&per_page=10")
        .reply(500, { message: "Internal Server Error" });
  ...
  describe("acceptInvitation", () => {
    ...
    it("should handle server error when accepting invitation", async () => {
      mockNamespacesApi
        .onPatch("http://localhost:3000/api/namespaces/tenant-123/invitations/accept")
        .reply(500, { message: "Internal Server Error" });

      await expect(store.acceptInvitation("tenant-123")).rejects.toBeAxiosErrorWithStatus(500);
    });
  ...
  describe("declineInvitation", () => {
    ...
    it("should handle server error when declining invitation", async () => {
      mockNamespacesApi
        .onPatch("http://localhost:3000/api/namespaces/tenant-123/invitations/decline")
        .reply(500, { message: "Internal Server Error" });

      await expect(store.declineInvitation("tenant-123")).rejects.toBeAxiosErrorWithStatus(500);
    });
  ...
  describe("editInvitation", () => {
    ...
    it("should handle server error when editing invitation", async () => {
      const editData = {
        tenant: "tenant-123",
        user_id: "user-456",
        role: "observer" as const,
      ...
      await expect(store.editInvitation(editData)).rejects.toBeAxiosErrorWithStatus(500);
    });
  ...
  describe("cancelInvitation", () => {
    ...
    it("should handle server error when canceling invitation", async () => {
      const cancelData = {
        tenant: "tenant-123",
        user_id: "user-456",
      };
      ...
      await expect(store.cancelInvitation(cancelData)).rejects.toBeAxiosErrorWithStatus(500);
    });
  ...
  describe("sendInvitationEmail", () => {
    ...
    it("should handle server error when sending invitation email", async () => {
      const inviteData = {
        email: "newuser@example.com",
        role: "observer" as const,
        tenant_id: "tenant-123",
      ...
      await expect(store.sendInvitationEmail(inviteData)).rejects.toBeAxiosErrorWithStatus(500);
    });
  ...
  describe("generateInvitationLink", () => {
    ...
    it("should handle server error when generating invitation link", async () => {
      const inviteData = {
        email: "newuser@example.com",
        role: "observer" as const,
        tenant_id: "tenant-123",
