...
describe("Billing Store", () => {
  ...
  describe("getSubscriptionInfo", () => {
    it("should fetch subscription info successfully when isCloud is true", async () => {
      ...
      // Mock isCloud to be true
      billingMock.onGet("http://localhost:3000/api/billing/subscription").reply(200, mockBillingBase);
    ...
    it("should not fetch subscription info when isCloud is false", async () => {
      ...
      billingMock.onGet("http://localhost:3000/api/billing/subscription").reply(200, { active: true });
    ...
    it("should set active to false when request fails with 400", async () => {
      const store = useBillingStore();

      billingMock.onGet("http://localhost:3000/api/billing/subscription").reply(400);
    ...
    it("should set active to false when request fails with 401", async () => {
      const store = useBillingStore();

      billingMock.onGet("http://localhost:3000/api/billing/subscription").reply(401);
    ...
    it("should set active to false when request fails with 403", async () => {
      const store = useBillingStore();

      billingMock.onGet("http://localhost:3000/api/billing/subscription").reply(403);
    ...
    it("should set active to false when request fails with 404", async () => {
      const store = useBillingStore();

      billingMock.onGet("http://localhost:3000/api/billing/subscription").reply(404);
    ...
    it("should set active to false when request fails with 500", async () => {
      const store = useBillingStore();

      billingMock.onGet("http://localhost:3000/api/billing/subscription").reply(500);
    ...
    it("should set active to false when network error occurs", async () => {
      ...
      billingMock.onGet("http://localhost:3000/api/billing/subscription").networkError();
  ...
  describe("openBillingPortal", () => {
    it("should open billing portal successfully", async () => {
      ...
      axiosMock.onPost("/api/billing/portal").reply(200, { url: mockUrl });
    ...
    it("should throw error when request fails with 401", async () => {
      const store = useBillingStore();

      axiosMock.onPost("/api/billing/portal").reply(401);

      await expect(store.openBillingPortal()).rejects.toBeAxiosErrorWithStatus(401);
    });

    it("should throw error when request fails with 403", async () => {
      const store = useBillingStore();

      axiosMock.onPost("/api/billing/portal").reply(403);

      await expect(store.openBillingPortal()).rejects.toBeAxiosErrorWithStatus(403);
    });

    it("should throw error when request fails with 404", async () => {
      const store = useBillingStore();

      axiosMock.onPost("/api/billing/portal").reply(404);

      await expect(store.openBillingPortal()).rejects.toBeAxiosErrorWithStatus(404);
    });

    it("should throw error when request fails with 500", async () => {
      const store = useBillingStore();

      axiosMock.onPost("/api/billing/portal").reply(500);

      await expect(store.openBillingPortal()).rejects.toBeAxiosErrorWithStatus(500);
    });

    it("should throw error when network error occurs", async () => {
      const store = useBillingStore();

      axiosMock.onPost("/api/billing/portal").networkError();
