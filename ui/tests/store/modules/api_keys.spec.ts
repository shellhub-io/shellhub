import { describe, expect, it, beforeEach, afterEach } from "vitest";
import MockAdapter from "axios-mock-adapter";
import { createPinia, setActivePinia } from "pinia";
import { apiKeysApi } from "@/api/http";
import useApiKeysStore from "@/store/modules/api_keys";

describe("API Keys Store", () => {
  let mockApiKeysApi: MockAdapter;
  let store: ReturnType<typeof useApiKeysStore>;

  beforeEach(() => {
    setActivePinia(createPinia());
    mockApiKeysApi = new MockAdapter(apiKeysApi.getAxios());
    store = useApiKeysStore();
  });

  afterEach(() => {
    mockApiKeysApi.reset();
  });

  describe("Initial State", () => {
    it("should have empty API keys array", () => {
      expect(store.apiKeys).toEqual([]);
    });

    it("should have zero API keys count", () => {
      expect(store.apiKeysCount).toBe(0);
    });
  });

  describe("fetchApiKeys", () => {
    it("should fetch API keys successfully with pagination", async () => {
      const mockApiKeys = [
        {
          id: "3e5a5194-test-4a32-fake-7434c6d49df1",
          tenant_id: "fake-tenant",
          name: "my api key",
          expires_in: 1707958989,
        },
        {
          id: "a1b2c3d4-5678-90ef-ghij-klmnopqrstuv",
          tenant_id: "fake-tenant",
          name: "production key",
          expires_in: 1709958989,
        },
      ];

      mockApiKeysApi
        .onGet("http://localhost:3000/api/namespaces/api-key?page=1&per_page=10")
        .reply(200, mockApiKeys, {
          "x-total-count": "2",
        });

      await store.fetchApiKeys({ page: 1, perPage: 10 });

      expect(store.apiKeys).toEqual(mockApiKeys);
      expect(store.apiKeysCount).toBe(2);
    });

    it("should handle empty API keys response", async () => {
      mockApiKeysApi
        .onGet("http://localhost:3000/api/namespaces/api-key?page=1&per_page=10")
        .reply(200, [], {
          "x-total-count": "0",
        });

      await store.fetchApiKeys({ page: 1, perPage: 10 });

      expect(store.apiKeys).toEqual([]);
      expect(store.apiKeysCount).toBe(0);
    });

    it("should fetch API keys with sorting parameters", async () => {
      const mockApiKeys = [
        {
          id: "key-1",
          tenant_id: "tenant",
          name: "api key 1",
          expires_in: 1707958989,
        },
      ];

      mockApiKeysApi
        .onGet("http://localhost:3000/api/namespaces/api-key?page=1&per_page=10&order_by=asc&sort_by=name")
        .reply(200, mockApiKeys, {
          "x-total-count": "1",
        });

      await store.fetchApiKeys({
        page: 1,
        perPage: 10,
        sortField: "name",
        sortOrder: "asc",
      });

      expect(store.apiKeys).toEqual(mockApiKeys);
      expect(store.apiKeysCount).toBe(1);
    });

    it("should handle API errors when fetching", async () => {
      mockApiKeysApi
        .onGet("http://localhost:3000/api/namespaces/api-key?page=1&per_page=10")
        .reply(500, { message: "Internal Server Error" });

      await expect(
        store.fetchApiKeys({ page: 1, perPage: 10 }),
      ).rejects.toBeAxiosErrorWithStatus(500);
    });

    it("should handle network errors", async () => {
      mockApiKeysApi
        .onGet("http://localhost:3000/api/namespaces/api-key?page=1&per_page=10")
        .networkError();

      await expect(
        store.fetchApiKeys({ page: 1, perPage: 10 }),
      ).rejects.toThrow();
    });
  });

  describe("generateApiKey", () => {
    it("should generate API key successfully", async () => {
      const mockGeneratedId = "c629572a-b643-4301-90fe-4572b00d007e";
      const requestData = {
        name: "dev",
        expires_in: 30,
        role: "administrator",
      };

      mockApiKeysApi
        .onPost("http://localhost:3000/api/namespaces/api-key")
        .reply(200, { id: mockGeneratedId });

      const result = await store.generateApiKey(requestData);

      expect(result).toBe(mockGeneratedId);
    });

    it("should handle validation errors when generating", async () => {
      const requestData = {
        name: "",
        expires_in: -1,
        role: "invalid",
      };

      mockApiKeysApi
        .onPost("http://localhost:3000/api/namespaces/api-key")
        .reply(400, { message: "Invalid request data" });

      await expect(
        store.generateApiKey(requestData),
      ).rejects.toBeAxiosErrorWithStatus(400);
    });

    it("should handle permission errors", async () => {
      const requestData = {
        name: "test key",
        expires_in: 30,
        role: "administrator",
      };

      mockApiKeysApi
        .onPost("http://localhost:3000/api/namespaces/api-key")
        .reply(403, { message: "Forbidden" });

      await expect(
        store.generateApiKey(requestData),
      ).rejects.toBeAxiosErrorWithStatus(403);
    });
  });

  describe("editApiKey", () => {
    it("should edit API key name successfully", async () => {
      const editData = {
        key: "test-key-123",
        name: "updated name",
        role: "administrator",
      };

      mockApiKeysApi
        .onPatch("http://localhost:3000/api/namespaces/api-key/test-key-123")
        .reply(200);

      await expect(store.editApiKey(editData)).resolves.not.toThrow();
    });

    it("should edit API key role successfully", async () => {
      const editData = {
        key: "test-key-456",
        name: "my key",
        role: "observer",
      };

      mockApiKeysApi
        .onPatch("http://localhost:3000/api/namespaces/api-key/test-key-456")
        .reply(200);

      await expect(store.editApiKey(editData)).resolves.not.toThrow();
    });

    it("should handle not found error when editing", async () => {
      const editData = {
        key: "non-existent-key",
        name: "test",
        role: "administrator",
      };

      mockApiKeysApi
        .onPatch("http://localhost:3000/api/namespaces/api-key/non-existent-key")
        .reply(404, { message: "API key not found" });

      await expect(
        store.editApiKey(editData),
      ).rejects.toBeAxiosErrorWithStatus(404);
    });

    it("should handle permission errors when editing", async () => {
      const editData = {
        key: "test-key",
        name: "new name",
        role: "administrator",
      };

      mockApiKeysApi
        .onPatch("http://localhost:3000/api/namespaces/api-key/test-key")
        .reply(403, { message: "Insufficient permissions" });

      await expect(
        store.editApiKey(editData),
      ).rejects.toBeAxiosErrorWithStatus(403);
    });
  });

  describe("removeApiKey", () => {
    it("should remove API key successfully", async () => {
      const removeData = {
        key: "test-key-to-remove",
      };

      mockApiKeysApi
        .onDelete("http://localhost:3000/api/namespaces/api-key/test-key-to-remove")
        .reply(200);

      await expect(store.removeApiKey(removeData)).resolves.not.toThrow();
    });

    it("should handle not found error when removing", async () => {
      const removeData = {
        key: "non-existent-key",
      };

      mockApiKeysApi
        .onDelete("http://localhost:3000/api/namespaces/api-key/non-existent-key")
        .reply(404, { message: "API key not found" });

      await expect(
        store.removeApiKey(removeData),
      ).rejects.toBeAxiosErrorWithStatus(404);
    });

    it("should handle permission errors when removing", async () => {
      const removeData = {
        key: "protected-key",
      };

      mockApiKeysApi
        .onDelete("http://localhost:3000/api/namespaces/api-key/protected-key")
        .reply(403, { message: "Cannot remove this API key" });

      await expect(
        store.removeApiKey(removeData),
      ).rejects.toBeAxiosErrorWithStatus(403);
    });

    it("should handle server errors when removing", async () => {
      const removeData = {
        key: "test-key",
      };

      mockApiKeysApi
        .onDelete("http://localhost:3000/api/namespaces/api-key/test-key")
        .reply(500, { message: "Internal Server Error" });

      await expect(
        store.removeApiKey(removeData),
      ).rejects.toBeAxiosErrorWithStatus(500);
    });
  });
});
