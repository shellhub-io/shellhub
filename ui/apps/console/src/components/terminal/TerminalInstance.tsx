    ...
    async function connect() {
      ...
      const body: Record<string, string | number> = isConnect
        ? {
            ...
            known_host_key: session.knownHostKey ?? "",
          }
        : { device: session.deviceUid, username: session.username };
      // Key auth (both paths): send the fingerprint so the server proxies the
      // signing challenge back to the browser; for direct connections it also
      // needs the public key to advertise (the external host isn't registered).
      if (session.fingerprint) {
        body.fingerprint = session.fingerprint;
        if (isConnect) body.public_key = session.publicKey ?? "";
      } else {
