...
describe("usePublicKeys", () => {
  describe("returns", () => {
    ...
    it("defaults publicKeys to empty array while loading", () => {
      sdk.getPublicKeys.mockReturnValue(new Promise(() => {}));
    ...
    it("exposes error when query fails", async () => {
      ...
      sdk.getPublicKeys.mockRejectedValue(networkError);
  ...
  describe("pagination defaults", () => {
    it("uses page 1 and perPage 10 as defaults", async () => {
      ...
      await waitFor(() => expect(sdk.getPublicKeys).toHaveBeenCalled());
      const [opts] = sdk.getPublicKeys.mock.calls[0];
