...
describe("useAdminUsers", () => {
  describe("when user is admin", () => {
    ...
    it("exposes error when query fails", async () => {
      ...
      sdk.getUsers.mockRejectedValue(networkError);
  ...
  describe("search filter", () => {
    it("passes search parameter to the query options", async () => {
      ...
      await waitFor(() => expect(result.current.isLoading).toBe(false));
      expect(sdk.getUsers).toHaveBeenCalled();
    });
    ...
    it("does not pass filter when search is empty", async () => {
      ...
      await waitFor(() => expect(sdk.getUsers).toHaveBeenCalled());
      const [opts] = sdk.getUsers.mock.calls[0];
...
describe("useAdminUser", () => {
  describe("when user is admin", () => {
    it("returns query data for the given user id", async () => {
      ...
      sdk.getUser.mockResolvedValue(mockSdkResponse(user));
    ...
    it("is loading initially when id is provided", () => {
      sdk.getUser.mockReturnValue(new Promise(() => {}));
    ...
    it("exposes error when query fails", async () => {
      ...
      sdk.getUser.mockRejectedValue(err);
