...
beforeEach(() => {
  vi.clearAllMocks();
  mockGetSessions.mockResolvedValue(paginatedResponse([]));
  mockCloseSession.mockResolvedValue(mockSdkResponse(undefined));
  mockGetSessionRecord.mockRejectedValue(new Error("no recording"));
});
...
describe("Sessions", () => {
  ...
  describe("play recording", () => {
    it("fetches the recording when Play is clicked", async () => {
      const user = userEvent.setup();
      mockGetSessionRecord.mockResolvedValue(mockSdkResponse("asciicast-data"));
      mockGetSessions.mockResolvedValue(
    ...
    it("disables the play button while the recording is loading", async () => {
      const user = userEvent.setup();
      mockGetSessionRecord.mockReturnValue(new Promise(() => {}));
      mockGetSessions.mockResolvedValue(
        paginatedResponse([mockSession({ uid: "session-1", recorded: true })]),
      );
      renderSessions();
