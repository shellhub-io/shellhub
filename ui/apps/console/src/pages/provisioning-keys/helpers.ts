 ...
 */
export function isSystemKey(key: ProvisioningKey): boolean {
  return key.type === "legacy" || key.type === "pairing";
}

/** The pairing system key specifically. */
export function isPairingKey(key: ProvisioningKey): boolean {
...
export function provisioningKeyDisplayName(key: ProvisioningKey): string {
...
/**
 * How a device came to be enrolled. Legacy and pairing predate provisioning keys, so neither has a
...
/**
 * Resolve a device's enrollment source by matching its `provisioning_key_id` digest
 * against the namespace's provisioning keys: the pairing system key → pairing, the
 * legacy system key → legacy, a real key → its name, no digest or no match →
 * null (render as "—").
 */
export function resolveEnrollmentSource(
  provisioningKeyId: string | undefined,
  provisioningKeys: ProvisioningKey[],
): EnrollmentSource | null {
  if (!provisioningKeyId) return null;
  const match = provisioningKeys.find((k) => k.id === provisioningKeyId);
...
/** Client-side name validation for a provisioning key: 3-20 chars, alphanumerics plus - and _. */
...
function isProvisioningKeyExpired(key: ProvisioningKey): boolean {
  return (
...
/**
 * The reasons a provisioning key will not enrol anything. They are independent and can hold at once,
...
export function getKeyBlockers(key: ProvisioningKey): KeyBlockers {
  ...
  const expired = isProvisioningKeyExpired(key);
...
export function getWaitingInfo(key: ProvisioningKey): WaitingInfo {
 ...
 */
export function getUsageInfo(key: ProvisioningKey): UsageInfo {
