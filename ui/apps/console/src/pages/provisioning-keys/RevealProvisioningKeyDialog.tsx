...
import { provisioningKeyDisplayName } from "./helpers";
 ...
 * useRevealProvisioningKey) and is dropped from cache on close so it doesn't linger. The legacy/system key
 * has no secret, so it shows only its fingerprint.
 */
export default function RevealProvisioningKeyDialog({
  provisioningKey,
  onClose,
}: {
  ...
  const autoId = useId();
  const titleId = `reveal-provisioning-key-title-${autoId}`;
  const fingerprintLabelId = `reveal-provisioning-key-fingerprint-${autoId}`;
  const secretLabelId = `reveal-provisioning-key-secret-${autoId}`;

  const name = provisioningKey?.name ?? null;
  const displayName = provisioningKey ? provisioningKeyDisplayName(provisioningKey) : name;
  const hasSecret = !!provisioningKey?.key_hint;
  const fingerprint = provisioningKey?.id ?? "";
  ...
  const { key, isLoading, error } = useRevealProvisioningKey(
    hasSecret ? name : null,
    revealed,
  );
