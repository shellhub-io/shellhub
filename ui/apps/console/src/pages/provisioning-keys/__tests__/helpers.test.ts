...
function key(overrides: Partial<ProvisioningKey>): ProvisioningKey {
  return {
    revoked: false,
    disabled: false,
    expires_at: null,
...
describe("enrollment source", () => {
  ...
  it("labels the pairing system key apart from the legacy one", () => {
    ...
    expect(provisioningKeyDisplayName(pairing)).toBe("Pairing code");
    expect(provisioningKeyDisplayName(legacy)).toBe("Tenant-only registration");
    expect(provisioningKeyDisplayName(real)).toBe("fleet");
  });
});
