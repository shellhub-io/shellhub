...
const mockListAnnouncementsAdmin = vi.hoisted(() => vi.fn());
...
vi.mock("@/client/sdk.gen", async (importOriginal) => {
  ...
  return { ...actual, listAnnouncementsAdmin: mockListAnnouncementsAdmin };
});
...
describe("AdminAnnouncements", () => {
  beforeEach(() => {
    ...
    mockListAnnouncementsAdmin.mockResolvedValue(paginatedResponse([]));
  });
  ...
  describe("loading state", () => {
    it("renders the loading spinner with role='status'", () => {
      mockListAnnouncementsAdmin.mockReturnValue(new Promise(() => {}));
      renderPage();
  ...
  describe("announcement rows", () => {
    it("renders a row for each returned announcement", async () => {
      mockListAnnouncementsAdmin.mockResolvedValue(
        paginatedResponse([
          mockAnnouncement({
            uuid: "uuid-a1b2",
  ...
  describe("error state", () => {
    it("renders an error alert when the SDK returns an error", async () => {
      mockListAnnouncementsAdmin.mockRejectedValue(makeSdkError(500));
      renderPage();
  ...
  describe("URL hydration (usePaginatedListState)", () => {
    it("passes page=2 to the SDK when URL has ?page=2", async () => {
      renderPage(["/?page=2"]);
      await waitFor(() => {
        expect(mockListAnnouncementsAdmin).toHaveBeenCalledWith(
  ...
  describe("delete-last-row page decrement (usePaginatedListState)", () => {
    it("decrements page from 2 to 1 via URL when deleting the last item on a page", async () => {
      ...
      mockListAnnouncementsAdmin.mockClear();
