...
const mockGetDeviceAdmin = vi.hoisted(() => vi.fn());

vi.mock("@/client/sdk.gen", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/client/sdk.gen")>();
  return { ...actual, getDeviceAdmin: mockGetDeviceAdmin };
});
...
beforeEach(() => {
  ...
  mockGetDeviceAdmin.mockResolvedValue(mockSdkResponse(makeDevice()));
});

describe("AdminDeviceDetails", () => {
  describe("loading state", () => {
    it('announces "Loading device details" while loading', () => {
      mockGetDeviceAdmin.mockReturnValue(new Promise(() => {}));
      renderPage();
      expect(
  ...
  describe("not-found / error state", () => {
    it('renders "Device not found" when no data and no loading', async () => {
      mockGetDeviceAdmin.mockRejectedValue(makeSdkError(404));
      renderPage();
      await waitFor(() => {
    ...
    it('renders "Device not found" when the query returns an error', async () => {
      mockGetDeviceAdmin.mockRejectedValue(makeSdkError(500));
      renderPage();
      await waitFor(() => {
        expect(screen.getByText("Device not found")).toBeInTheDocument();
      });
  ...
  describe("device data", () => {
    ...
    it('renders "No tags" when device has no tags', async () => {
      mockGetDeviceAdmin.mockResolvedValue(
