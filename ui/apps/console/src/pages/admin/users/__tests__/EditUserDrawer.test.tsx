...
vi.mock("@/client/sdk.gen", async (importOriginal) => {
  ...
  return { ...actual, adminUpdateUser: mockAdminUpdateUser };
});
...
describe("EditUserDrawer", () => {
  ...
  describe("submit — error handling", () => {
    it("shows conflict error message for 409 responses", async () => {
      mockAdminUpdateUser.mockRejectedValue({ status: 409 });
      renderDrawer();
    ...
    it("shows generic error for 400 responses", async () => {
      mockAdminUpdateUser.mockRejectedValue({ status: 400 });
      renderDrawer();
    ...
    it("shows generic error for unexpected failures", async () => {
      mockAdminUpdateUser.mockRejectedValue(new Error("server error"));
      renderDrawer();
  ...
  describe("client-side validation", () => {
    it("allows submit with a blank password (password kept as-is)", async () => {
      ...
      await waitFor(() => expect(mockAdminUpdateUser).toHaveBeenCalled());
    });
  ...
  describe("state reset on reopen", () => {
    ...
    it("clears any error when closed then reopened", async () => {
      mockAdminUpdateUser.mockRejectedValue(new Error("fail"));
      const { rerender } = renderDrawer({ user: mockUser });
