...
beforeEach(() => {
  ...
  mockGetLicense.mockResolvedValue(mockSdkResponse({}));
...
describe("AdminLicense", () => {
  ...
  describe("status alerts", () => {
    ...
    it("shows info alert when about_to_expire", async () => {
      mockGetLicense.mockResolvedValue(mockSdkResponse(aboutToExpireLicense));
      renderPage();
      expect(
    ...
    it("shows warning when expired + grace period", async () => {
      mockGetLicense.mockResolvedValue(mockSdkResponse(gracePeriodLicense));
      renderPage();
      expect(await screen.findByRole("alert")).toBeInTheDocument();
    ...
    it("shows error when expired without grace period", async () => {
      mockGetLicense.mockResolvedValue(mockSdkResponse(expiredLicense));
      renderPage();
      expect(
    ...
    it("shows no alert when license is valid", async () => {
      mockGetLicense.mockResolvedValue(mockSdkResponse(validLicense));
      renderPage();
  ...
  describe("license details", () => {
    ...
    it("shows 'Now' for -1 timestamps", async () => {
      ...
      mockGetLicense.mockResolvedValue(mockSdkResponse(licenseWithNow));
      renderPage();
      await screen.findByText("License Information");
    ...
    it("shows region list when regions are non-empty", async () => {
      mockGetLicense.mockResolvedValue(mockSdkResponse(regionalLicense));
      renderPage();
