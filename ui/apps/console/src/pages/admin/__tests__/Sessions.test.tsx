...
beforeEach(() => {
  ...
  mockGetSessionsAdmin.mockResolvedValue(paginatedResponse([]));
});
...
describe("AdminSessions", () => {
  ...
  describe("error state", () => {
    it("renders the error banner with role='alert'", async () => {
      mockGetSessionsAdmin.mockRejectedValue(makeSdkError(500));
      renderPage();
      expect(await screen.findByRole("alert")).toBeInTheDocument();
    });

    it("displays the console's own copy for the status in the banner", async () => {
      mockGetSessionsAdmin.mockRejectedValue(makeSdkError(403));
      renderPage();
  ...
  describe("URL hydration", () => {
    it("passes page=3 to the SDK when URL has ?page=3", async () => {
      renderPage(["/?page=3"]);
      await waitFor(() => {
        expect(mockGetSessionsAdmin).toHaveBeenCalledWith(
