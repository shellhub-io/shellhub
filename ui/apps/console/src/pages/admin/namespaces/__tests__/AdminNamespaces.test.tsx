...
const mockGetNamespacesAdmin = vi.hoisted(() => vi.fn());

vi.mock("@/client/sdk.gen", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/client/sdk.gen")>();
  return { ...actual, getNamespacesAdmin: mockGetNamespacesAdmin };
});
...
describe("AdminNamespaces", () => {
  beforeEach(() => {
    ...
    mockGetNamespacesAdmin.mockResolvedValue(paginatedResponse([]));
  });
  ...
  describe("loading state", () => {
    it('renders the loading spinner with "Loading namespaces..." text', () => {
      mockGetNamespacesAdmin.mockReturnValue(new Promise(() => {}));
      renderPage();
  ...
  describe("namespace rows", () => {
    it("renders a row for each returned namespace", async () => {
      mockGetNamespacesAdmin.mockResolvedValue(
        paginatedResponse([
  ...
  describe("error state", () => {
    it("renders an error alert when the SDK returns an error", async () => {
      mockGetNamespacesAdmin.mockRejectedValue(makeSdkError(500));
      renderPage();
  ...
  describe("URL hydration — controls reflect URL params on mount", () => {
    it("passes search and page hydrated from URL to the SDK", async () => {
      ...
      await waitFor(() => {
        expect(mockGetNamespacesAdmin).toHaveBeenCalledWith(
  ...
  describe("debounce — search is debounced before reaching the query hook", () => {
    ...
    it("does not pass the new search to the SDK until the debounce delay elapses", async () => {
      ...
      expect(hasFilter(mockGetNamespacesAdmin.mock.calls)).toBe(false);
      ...
      await waitFor(() => {
        expect(hasFilter(mockGetNamespacesAdmin.mock.calls)).toBe(true);
      });
    });
  });
});
