...
const mockDeleteNamespaceAdmin = vi.hoisted(() => vi.fn());

vi.mock("@/client/sdk.gen", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/client/sdk.gen")>();
  return { ...actual, deleteNamespaceAdmin: mockDeleteNamespaceAdmin };
});
...
beforeEach(() => {
  vi.clearAllMocks();
  mockDeleteNamespaceAdmin.mockResolvedValue(mockSdkResponse(undefined));
});
...
describe("DeleteNamespaceDialog", () => {
  ...
  describe("confirm — success", () => {
    it("calls deleteNamespaceAdmin with the correct tenant_id", async () => {
      ...
      await waitFor(() => {
        expect(mockDeleteNamespaceAdmin).toHaveBeenCalledWith(
  ...
  describe("confirm — error handling", () => {
    it("shows generic error message on failure", async () => {
      mockDeleteNamespaceAdmin.mockRejectedValue(new Error("server error"));
      renderDialog();
    ...
    it("shows error for SDK errors", async () => {
      mockDeleteNamespaceAdmin.mockRejectedValue({ status: 500 });
      renderDialog();
  ...
  describe("cancel", () => {
    ...
    it("does not call deleteNamespaceAdmin when Cancel is clicked", async () => {
      renderDialog();
      await userEvent.click(screen.getByRole("button", { name: /cancel/i }));
      expect(mockDeleteNamespaceAdmin).not.toHaveBeenCalled();
    });
