...
const mockCreateFirewallRule = vi.hoisted(() => vi.fn());
const mockUpdateFirewallRule = vi.hoisted(() => vi.fn());
...
beforeEach(() => {
  vi.clearAllMocks();
  mockCreateFirewallRule.mockResolvedValue(mockSdkResponse(undefined));
  mockUpdateFirewallRule.mockResolvedValue(mockSdkResponse(undefined));
...
describe("RuleDrawer — create mode", () => {
  ...
  it("calls createFirewallRule with the correct body and calls onClose on success", async () => {
    ...
    await waitFor(() =>
      expect(mockCreateFirewallRule).toHaveBeenCalledWith(
...
describe("RuleDrawer — API rejection", () => {
  it("shows root error alert and does not call onClose when API rejects", async () => {
    const user = userEvent.setup();
    mockCreateFirewallRule.mockRejectedValue(new Error("Server error"));
    const onClose = vi.fn();
    renderDrawer({ onClose });
