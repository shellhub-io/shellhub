...
const mockGetFirewallRules = vi.hoisted(() => vi.fn());
const mockDeleteFirewallRule = vi.hoisted(() => vi.fn());
...
beforeEach(() => {
  ...
  mockGetFirewallRules.mockResolvedValue(
    paginatedResponse([mockFirewallRule({ priority: 42 })]),
  );
  mockDeleteFirewallRule.mockResolvedValue(mockSdkResponse(undefined));
});
...
describe("FirewallRules — delete error handling", () => {
  ...
  it("shows the mutation error message inside the dialog when deletion fails", async () => {
    mockDeleteFirewallRule.mockRejectedValue(new Error("Permission denied"));
  ...
  it("shows a generic fallback message when the rejection is not an Error", async () => {
    mockDeleteFirewallRule.mockRejectedValue("boom");
    const user = await openDeleteDialog();
    const dialog = await getDialog();
  ...
  it("clears any previous error when the dialog is cancelled and reopened", async () => {
    mockDeleteFirewallRule.mockRejectedValueOnce(new Error("Transient"));
    const user = await openDeleteDialog();
...
describe("FirewallRules — URL hydration", () => {
  ...
  it("hydrates page from URL and passes it to the SDK", async () => {
    renderPage(["/?page=3"]);
    await waitFor(() => {
      expect(mockGetFirewallRules).toHaveBeenCalledWith(
  ...
  it("setSearch resets page to 1 in the URL", async () => {
    ...
    await waitFor(() => {
      const calls = mockGetFirewallRules.mock.calls;
