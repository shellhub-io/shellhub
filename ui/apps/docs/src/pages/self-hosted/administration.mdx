...
> **Note:** The password can contain any characters, but its length must be between 5 and 32 characters.
...
An optional third argument sets the tenant ID (a UUID, generated automatically when omitted). The `--type` flag sets the namespace type (`team` or `personal`, default: `team`).

> **Note:** The namespace must be a valid [RFC1123](https://www.rfc-editor.org/rfc/rfc1123) hostname and must not contain dots.
...
- [Troubleshooting](/self-hosted/troubleshooting) — Fix common issues
