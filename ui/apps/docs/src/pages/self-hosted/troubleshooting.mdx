...
$ ./bin/docker-compose logs gateway     # Gateway
```
...
Users connect with: `ssh -p 2222 user@namespace.device@your-server.com`
...
**Option C:** Run ShellHub behind the existing reverse proxy. See [Configuration](/self-hosted/configuring).
...
3. Check disk space: `df -h` — Docker and PostgreSQL both need free disk space
...
See [Administration](/self-hosted/administration) for more CLI commands.

### Database connection errors

ShellHub uses PostgreSQL as its database backend (MongoDB was removed in v0.24.0):
...
4. Check disk space — PostgreSQL needs free space for its data files

> **Note:** Instances still running a pre-v0.24.0 release use MongoDB instead. Check `./bin/docker-compose logs mongo` and see [Upgrade](/self-hosted/upgrading) to migrate to PostgreSQL.
