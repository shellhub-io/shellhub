...
| `SHELLHUB_PROXY` | `false` | Accept the PROXY protocol on the gateway's listener |
| `SHELLHUB_PROXY_TRUSTED_IPS` | `0.0.0.0/0 ::/0` | Peers allowed to declare the client's address |
...
| `SHELLHUB_GATEWAY_ACCESS_LOGS` | `true` | Enable gateway access logs |
| `SHELLHUB_ACME_CA_SERVER` | | ACME directory to request certificates from; empty means Let's Encrypt |
...
| `SHELLHUB_EDITION` | `community` | Product edition: `community`, `enterprise`, or `cloud` |
...
> **Note:** With the Connector enabled, container access is restricted for security. Password authentication is denied for users without a password set in the container's `/etc/shadow` (set one with `passwd`); such users can still connect using public-key authentication.
