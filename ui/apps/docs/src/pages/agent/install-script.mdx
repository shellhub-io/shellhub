...
> **Note:** On WSL systems, the script **overrides** any previously detected method. If the kernel reports "Microsoft" in `/proc/version`, the script forces the WSL install path — even if Docker is available. On non-Ubuntu WSL, this causes an error exit. See the [WSL guide](/agent/wsl) for details.
...
Auto-update is **not available** for Snap, standalone, or [native builds](/agent/building-from-source). For Snap, use `snap refresh shellhub`. For standalone, re-run the install script.
