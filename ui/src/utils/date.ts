...
const formatDate = (date: string, formatToken: string) => date ? moment(date).format(formatToken) : "No date found";
...
export const formatUnixToDate = (timestamp: number) => timestamp ? moment.unix(timestamp).format("MMM Do YYYY") : "No date found";

/** Returns the time difference from the given date (e.g. "3 hours ago") */
export const getTimeFromNow = (date: string) => date ? moment(date).fromNow() : "No date found";
