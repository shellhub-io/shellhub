...
/* ---------- TAGS: only validate when Tags is the active filter; hard cap at 3 ---------- */
const setSelectedTagsError = () => {
  // Only enforce/validate when Tags filter is active
  if (selectedFilterOption.value !== FormFilterOptions.Tags) {
    selectedTagsError.value = "";
    return;
  }
  if (selectedTags.value.length > 3) {
    nextTick(() => selectedTags.value.pop()); // cap at 3
