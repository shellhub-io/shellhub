...
const extractTagName = (tag: Tag | string | unknown): string => {
...
const selectedTags = ref<Array<string>>(
  props.tagsList.map(extractTagName).filter((name) => name !== ""),
);
...
const open = async () => {
  ...
  perPage.value = 10;
  hasMore.value = true;
  selectedTags.value = props.tagsList.map(extractTagName).filter((name) => name !== "");
  await loadTags();
};
