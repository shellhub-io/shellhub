...
describe("Sessions", () => {
  ...
  it("Return device default variables", () => {
    expect(store.getters["sessions/sessions"]).toEqual([]);
    expect(store.getters["sessions/session"]).toEqual({});
  });

  it("Verify initial state change for setSessions mutation", () => {
    store.commit("sessions/setSessions", {
      data: sessions,
      headers: { "x-total-count": numberSessions },
    });
    expect(store.getters["sessions/sessions"]).toEqual(sessions);
    expect(store.getters["sessions/numberSessions"]).toEqual(numberSessions);
  });
  it("Verify inital state change for setSession mutation", () => {
    store.commit("sessions/setSession", { data: session });
    expect(store.getters["sessions/session"]).toEqual(session);
  });
