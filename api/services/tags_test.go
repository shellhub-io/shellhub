...
func TestGetTags(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			requiredMocks: func() {
				mock.On("NamespaceResolve", ctx, store.NamespaceTenantIDResolver, "not_found_tenant").Return(nil, errors.New("error", "", 0)).Once()
			},
			expected: Expected{
				Tags:  nil,
				Count: 0,
		...
		},
		{
			...
			requiredMocks: func() {
				...
				mock.On("NamespaceResolve", ctx, store.NamespaceTenantIDResolver, "tenant").Return(namespace, nil).Once()
