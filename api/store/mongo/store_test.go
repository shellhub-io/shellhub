...
func TestMain(m *testing.M) {
	...
	log.Info("Connecting to ", db.Container.ConnectionString)

	_, mdb, err := mongo.Connect(ctx, db.Container.ConnectionString+"/"+db.Container.Database)
	if err != nil {
		log.WithError(err).Error("Failed to connect to mongodb")
		os.Exit(1)
	}

	s, err = mongo.NewStore(ctx, mdb, cache.NewNullCache())
	if err != nil {
