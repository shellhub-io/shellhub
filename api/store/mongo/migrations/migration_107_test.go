...
func TestMigration107Up(t *testing.T) {
	...
	cases := []struct {
		...
		{
			description: "succeeds restructuring signon_url to signon_urls when SAML has signon_url",
			setup: func() error {
				system := bson.M{
			...
			verify: func(tt *testing.T) {
				...
				signonURLs, hasSignonURLs := idp["signon_urls"]
				require.True(tt, hasSignonURLs)

				signonURLsMap := signonURLs.(map[string]any)
				assert.Equal(tt, "https://example.com/saml/login", signonURLsMap["post"])
				assert.Equal(tt, "post", signonURLsMap["preferred_binding"])
				assert.Equal(tt, "", signonURLsMap["redirect"])
			},
		},
		{
			description: "creates signon_urls even when SAML config doesn't exist",
			setup: func() error {
				system := bson.M{
					"authentication": bson.M{
						"local": bson.M{
			...
			verify: func(tt *testing.T) {
				...
				signonURLs := idp["signon_urls"].(map[string]any)
				assert.Equal(tt, "", signonURLs["post"])
				assert.Equal(tt, "post", signonURLs["preferred_binding"])
				assert.Equal(tt, "", signonURLs["redirect"])
			},
		},
	}
