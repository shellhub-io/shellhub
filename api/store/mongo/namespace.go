...
func (s *Store) NamespaceDeleteMany(ctx context.Context, tenantIDs []string) (int64, error) {
	...
	fn := func(sCtx mongo.SessionContext) (any, error) {
		r, err := s.db.Collection("namespaces").DeleteMany(sCtx, bson.M{"tenant_id": bson.M{"$in": tenantIDs}})
		if err != nil {
			return 0, FromMongoError(err)
		}
		...
		for _, collection := range collections {
			if _, err := s.db.Collection(collection).DeleteMany(sCtx, bson.M{"tenant_id": bson.M{"$in": tenantIDs}}); err != nil {
				return 0, FromMongoError(err)
			}
		}
