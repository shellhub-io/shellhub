...
func (pg *pg) DeviceCreate(ctx context.Context, device *models.Device) (string, error) {
	...
	if device.Info != nil {
		e.Info.DeviceID = e.ID
		if _, err := pg.driver.NewInsert().Model(e.Info).Exec(ctx); err != nil {
			return "", fromSqlError(err)
		}
	}

	if device.Position != nil {
		e.Position.DeviceID = e.ID
		if _, err := pg.driver.NewInsert().Model(e.Position).Exec(ctx); err != nil {
			return "", fromSqlError(err)
		}
	}
