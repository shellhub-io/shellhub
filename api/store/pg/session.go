...
func (pg *Pg) SessionList(ctx context.Context, opts ...store.QueryOption) ([]models.Session, int, error) {
	...
	query = db.NewSelect().
		...
		Relation("Device.Namespace").
		ColumnExpr("sessions.*").
		ColumnExpr("CASE WHEN active_sessions.session_id IS NOT NULL THEN true ELSE false END as active").
		...
		Join(`LEFT JOIN (
			SELECT session_id, string_agg(DISTINCT type, ',') as types
			FROM session_events
			GROUP BY session_id
		) event_types ON sessions.id = event_types.session_id`).
		Join(`LEFT JOIN (
			SELECT session_id, string_agg(DISTINCT seat::text, ',') as seats
			FROM session_events
			GROUP BY session_id
		) event_seats ON sessions.id = event_seats.session_id`)
...
func (pg *Pg) SessionResolve(ctx context.Context, resolver store.SessionResolver, value string, opts ...store.QueryOption) (*models.Session, error) {
	...
	query := db.NewSelect().
		...
		Join(`LEFT JOIN (
			...
			WHERE session_id = ?
			GROUP BY session_id
		) event_types ON sessions.id = event_types.session_id`, sessionID).
		Join(`LEFT JOIN (
			SELECT session_id, string_agg(DISTINCT seat::text, ',') as seats
			FROM session_events
			WHERE session_id = ?
			GROUP BY session_id
		) event_seats ON sessions.id = event_seats.session_id`, sessionID).
