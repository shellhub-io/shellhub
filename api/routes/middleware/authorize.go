...
// returns [http.StatusForbidden]. Callers coming through the cloud admin
// panel bypass this check; they are identified by the /admin/api gateway,
// which strips X-ID and keeps X-Admin: true. An admin user who hits the
// regular /api/* surface still carries X-ID and is subject to the tenant
// guard so that a compromised or mis-scoped admin session cannot write
// across tenants from the user-facing API.
