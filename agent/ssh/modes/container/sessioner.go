...
// NOTE: Ensures the Sessioner interface is implemented.
var _ modes.Sessioner = new(Sessioner)
...
// is set later.
func NewSessioner(docker dockerclient.APIClient, container string) modes.Sessioner {
	return &Sessioner{
		container: container,
		docker:    docker,
	}
...
func (s *Sessioner) Shell(session gliderssh.Session) error {
	...
	if err != nil {
		logger.WithError(err).Error("Failed to attach shell to container")
	...
	go func() {
		...
		defer func() {
			...
			if err != nil {
				logger.WithError(err).Error("Failed to get exit code from container")
			}
		...
		if _, err := io.Copy(session, resp.Conn); err != nil && err != io.EOF {
			logger.WithError(err).Error("Failed to copy data from container to session")
		}
	}()
	...
	go func() {
		...
		if _, err := io.Copy(resp.Conn, session); err != nil && err != io.EOF {
			logger.WithError(err).Error("Failed to copy data from session to container")
		}
	}()
