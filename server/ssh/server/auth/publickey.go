...
func PublicKeyOffer(ctx gliderssh.Context, publicKey gliderssh.PublicKey) bool {
	logger := log.WithFields(
		log.Fields{
			"uid":   ctx.SessionID(),
			"sshid": ctx.User(),
			"key":   gossh.MarshalAuthorizedKey(publicKey),
		})
...
// was accepted, so this is where the session registration and the connection to
// the agent belong. Failing here is an ordinary authentication failure and
// counts against MaxAuthTries.
//
// It returns [session.ErrApprovalRequired] when the key is good but a person has
// to decide, and the caller then asks the client over keyboard-interactive
// rather than holding the connection open.
func PublicKeyVerified(ctx gliderssh.Context, publicKey gliderssh.PublicKey) error {
	logger := log.WithFields(
		log.Fields{
			"uid":   ctx.SessionID(),
			"sshid": ctx.User(),
	...
	if err := sess.Auth(ctx, auth); err != nil {
		if errors.Is(err, session.ErrApprovalRequired) {
			logger.Info("the presented key needs an approval before it can log in")
...
// PublicKeyOfferCallback adapts [PublicKeyOffer] to the shape x/crypto's
// ServerAuthCallbacks holds, so a refused approval can hand publickey back to a
// client that still has keys to try.
func PublicKeyOfferCallback(ctx gliderssh.Context) func(gossh.ConnMetadata, gossh.PublicKey) (*gossh.Permissions, error) {
	return func(_ gossh.ConnMetadata, key gossh.PublicKey) (*gossh.Permissions, error) {
		if ok := PublicKeyOffer(ctx, key); !ok {
