...
func TestUpdateProvisioningKey(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			requiredMocks: func() {
				svcMock.On("UpdateProvisioningKey", mock.Anything, mock.Anything).
					Return(services.NewErrProvisioningKeyInvalidField(map[string]string{
		...
		},
		{
			...
			requiredMocks: func() {
				svcMock.On("UpdateProvisioningKey", mock.Anything, mock.Anything).
					Return(services.NewErrProvisioningKeyForbidden()).
					Once()
			},
			expected: Expected{
				status: http.StatusForbidden,
				body:   &responses.Error{Message: "the legacy provisioning key cannot be modified"},
			},
		},
		{
	...
	for _, tc := range cases {
		t.Run(tc.description, func(t *testing.T) {
			...
			req := httptest.NewRequestWithContext(t.Context(), http.MethodPatch, "/api/namespaces/provisioning-key/ci", strings.NewReader(string(data)))
