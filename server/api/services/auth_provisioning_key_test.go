...
// TestAuthDevice_ProvisioningKey covers the provisioning-key enrollment decisions in AuthDevice: an invalid or
...
func TestAuthDevice_ProvisioningKey(t *testing.T) {
	...
	badDigest := hashProvisioningKey("bad-key")
	...
	cases := []struct {
		...
		{
			description: "rejects an invalid provisioning key without creating the device",
			req:         requests.DeviceAuth{TenantID: tenant, Hostname: "d", Identity: &requests.DeviceIdentity{MAC: "aa:bb:cc:dd:ee:ff"}, Info: &requests.DeviceInfo{}, PublicKey: "pk", ProvisioningKey: "bad-key"},
			requiredMocks: func(ctx context.Context) {
				...
				storeMock.On("ProvisioningKeyResolve", ctx, testifymock.Anything, store.ProvisioningKeyIDResolver, badDigest).Return(nil, store.ErrNoDocuments).Once()
			},
			expected: Expected{res: nil, err: NewErrAuthInvalid(map[string]any{"provisioning_key": "invalid"}, store.ErrNoDocuments)},
		},
		{
			...
			requiredMocks: func(ctx context.Context) {
				...
				cacheMock.On("Get", ctx, "auth_device/"+uid, testifymock.Anything).Return(nil).Once()
				storeMock.On("DeviceResolve", ctx, testifymock.Anything, store.DeviceUIDResolver, uid).Return(nil, store.ErrNoDocuments).Once()
				storeMock.On("ProvisioningKeyResolve", ctx, testifymock.Anything, store.ProvisioningKeyIDResolver, badDigest).Return(&models.ProvisioningKey{ID: badDigest, TenantID: tenant, Type: models.ProvisioningKeyTypeLegacy, Reusable: true}, nil).Once()
			},
			expected: Expected{res: nil, err: NewErrAuthInvalid(map[string]any{"provisioning_key": "invalid"}, nil)},
		},
		{
			...
			requiredMocks: func(ctx context.Context) {
				...
				cacheMock.On("Get", ctx, "auth_device/"+uid, testifymock.Anything).Return(nil).Once()
				storeMock.On("DeviceResolve", ctx, testifymock.Anything, store.DeviceUIDResolver, uid).Return(nil, store.ErrNoDocuments).Once()
				storeMock.On("ProvisioningKeyResolveSystem", ctx, scope.MustBounded(tenant)).Return(&models.ProvisioningKey{ID: "legacydigest", TenantID: tenant, Type: models.ProvisioningKeyTypeLegacy, Mode: models.ProvisioningKeyModeManual}, nil).Once()
				storeMock.On("DeviceCreate", ctx, &models.Device{
					...
					ProvisioningKeyID: "legacydigest",
				}).Return(uid, nil).Once()
				...
				storeMock.On("ProvisioningKeyEventCreate", ctx, testifymock.Anything).Return(nil).Once()
...
// TestEnrollmentProvisioningKey covers keyless enrollment-source resolution: the pairing-code flow (paired)
...
func TestEnrollmentProvisioningKey(t *testing.T) {
	const tenant = "00000000-0000-4000-0000-000000000000"

	legacy := &models.ProvisioningKey{ID: "legacydigest", TenantID: tenant, Type: models.ProvisioningKeyTypeLegacy, Mode: models.ProvisioningKeyModeManual}
	pairing := &models.ProvisioningKey{ID: "pairingdigest", TenantID: tenant, Type: models.ProvisioningKeyTypePairing, Mode: models.ProvisioningKeyModeAutomatic}
...
// TestAuthDevice_ProvisioningKeyWithoutTenant covers the enrollment of an agent that presents a provisioning
...
func TestAuthDevice_ProvisioningKeyWithoutTenant(t *testing.T) {
	storeMock := mocks.NewMockStore(t)
	queryOptionsMock := mocks.NewMockQueryOptions(t)
	storeMock.On("Options").Return(queryOptionsMock).Maybe()
	cacheMock := mockcache.NewMockCache(t)
	...
	unbounded := scope.NewUnbounded(reasonProvisioningKeyTenant)
	keyDigest := hashProvisioningKey("fleet-key")
	unknownDigest := hashProvisioningKey("no-such-key")
	...
	cases := []struct {
	...
	}{
		{
			...
			req:         requests.DeviceAuth{Hostname: "fleet-a", Identity: &requests.DeviceIdentity{MAC: "aa:bb:cc:dd:ee:01"}, Info: &requests.DeviceInfo{}, PublicKey: "pk", ProvisioningKey: "fleet-key"},
			requiredMocks: func(ctx context.Context) {
				uid := toUID("fleet-a", "aa:bb:cc:dd:ee:01", "pk")
				key := &models.ProvisioningKey{ID: keyDigest, TenantID: tenant, Type: models.ProvisioningKeyTypeUser, Mode: models.ProvisioningKeyModeManual, Reusable: true}
				storeMock.On("ProvisioningKeyResolve", ctx, unbounded, store.ProvisioningKeyIDResolver, keyDigest).Return(key, nil).Once()
				storeMock.On("NamespaceResolve", ctx, store.NamespaceTenantIDResolver, tenant).Return(&models.Namespace{TenantID: tenant, Name: "test"}, nil).Once()
				cacheMock.On("Get", ctx, "auth_device/"+uid, testifymock.Anything).Return(nil).Once()
				storeMock.On("DeviceResolve", ctx, testifymock.Anything, store.DeviceUIDResolver, uid).Return(nil, store.ErrNoDocuments).Once()
				storeMock.On("ProvisioningKeyResolve", ctx, scope.MustBounded(tenant), store.ProvisioningKeyIDResolver, keyDigest).Return(key, nil).Once()
				storeMock.On("DeviceCreate", ctx, &models.Device{
					CreatedAt:         now,
					UID:               uid,
					TenantID:          tenant,
		...
		},
		{
			...
			req:         requests.DeviceAuth{Hostname: "fleet-b", Identity: &requests.DeviceIdentity{MAC: "aa:bb:cc:dd:ee:02"}, Info: &requests.DeviceInfo{}, PublicKey: "pk", ProvisioningKey: "no-such-key"},
			requiredMocks: func(ctx context.Context) {
				storeMock.On("ProvisioningKeyResolve", ctx, unbounded, store.ProvisioningKeyIDResolver, unknownDigest).Return(nil, store.ErrNoDocuments).Once()
			},
			expected: Expected{res: nil, err: NewErrAuthInvalid(map[string]any{"provisioning_key": "invalid"}, store.ErrNoDocuments)},
		},
		{
			description: "refuses a system key presented as the only credential",
			req:         requests.DeviceAuth{Hostname: "fleet-c", Identity: &requests.DeviceIdentity{MAC: "aa:bb:cc:dd:ee:03"}, Info: &requests.DeviceInfo{}, PublicKey: "pk", ProvisioningKey: "fleet-key"},
			requiredMocks: func(ctx context.Context) {
				legacy := &models.ProvisioningKey{ID: keyDigest, TenantID: tenant, Type: models.ProvisioningKeyTypeLegacy, Mode: models.ProvisioningKeyModeManual}
				storeMock.On("ProvisioningKeyResolve", ctx, unbounded, store.ProvisioningKeyIDResolver, keyDigest).Return(legacy, nil).Once()
			},
			expected: Expected{res: nil, err: NewErrAuthInvalid(map[string]any{"provisioning_key": "invalid"}, nil)},
		},
	}
...
// TestProvisioningKeyTenant covers the contract that recovering a namespace from a key is not authorizing
// an enrollment: a revoked key still names its namespace, so a device it already enrolled keeps
// reconnecting, while enrollmentProvisioningKey remains the gate that refuses it a new one.
func TestProvisioningKeyTenant(t *testing.T) {
	const tenant = "00000000-0000-4000-0000-000000000000"

	unbounded := scope.NewUnbounded(reasonProvisioningKeyTenant)
	digest := hashProvisioningKey("fleet-key")
	...
	cases := []struct {
	...
	}{
		{
			...
			requiredMocks: func(ctx context.Context, storeMock *mocks.MockStore) {
				key := &models.ProvisioningKey{ID: digest, TenantID: tenant, Type: models.ProvisioningKeyTypeUser, Reusable: true}
				storeMock.On("ProvisioningKeyResolve", ctx, unbounded, store.ProvisioningKeyIDResolver, digest).Return(key, nil).Once()
			},
		...
		},
		{
			...
			requiredMocks: func(ctx context.Context, storeMock *mocks.MockStore) {
				key := &models.ProvisioningKey{ID: digest, TenantID: tenant, Type: models.ProvisioningKeyTypeUser, Reusable: true, Revoked: true}
				storeMock.On("ProvisioningKeyResolve", ctx, unbounded, store.ProvisioningKeyIDResolver, digest).Return(key, nil).Once()
			},
			expected: Expected{tenantID: tenant, err: nil},
		},
		{
			...
			requiredMocks: func(ctx context.Context, storeMock *mocks.MockStore) {
				key := &models.ProvisioningKey{ID: digest, TenantID: tenant, Type: models.ProvisioningKeyTypePairing}
				storeMock.On("ProvisioningKeyResolve", ctx, unbounded, store.ProvisioningKeyIDResolver, digest).Return(key, nil).Once()
			},
			expected: Expected{tenantID: "", err: NewErrAuthInvalid(map[string]any{"provisioning_key": "invalid"}, nil)},
		},
		{
			description: "refuses an unresolvable key",
			requiredMocks: func(ctx context.Context, storeMock *mocks.MockStore) {
				storeMock.On("ProvisioningKeyResolve", ctx, unbounded, store.ProvisioningKeyIDResolver, digest).Return(nil, store.ErrNoDocuments).Once()
			},
			expected: Expected{tenantID: "", err: NewErrAuthInvalid(map[string]any{"provisioning_key": "invalid"}, store.ErrNoDocuments)},
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.description, func(tt *testing.T) {
			...
			tenantID, err := s.provisioningKeyTenant(ctx, "fleet-key")
