...
func TestNewErrors(t *testing.T) {
	cases := []struct {
		...
		{
			description:  "a route forbidden error yields 403 and its message",
			err:          routes.NewErrForbidden(nil),
			wantStatus:   http.StatusForbidden,
			wantBody:     responses.Error{Message: "forbidden"},
			wantReported: false,
		},
		{
			description:  "a route not-found error yields 404 and its message",
			err:          routes.NewErrNotFound(nil),
			wantStatus:   http.StatusNotFound,
			wantBody:     responses.Error{Message: "not found"},
			wantReported: false,
		},
		{
			description:  "a route too-many-requests error yields 429 and its message",
			err:          routes.NewErrTooManyRequests(nil),
			wantStatus:   http.StatusTooManyRequests,
			wantBody:     responses.Error{Message: "too many requests"},
			wantReported: false,
		},
		{
			description:  "a route payment-required error yields 402 and its message",
			err:          routes.NewErrPaymentRequired(nil),
			wantStatus:   http.StatusPaymentRequired,
			wantBody:     responses.Error{Message: "payment required"},
			wantReported: false,
		},
