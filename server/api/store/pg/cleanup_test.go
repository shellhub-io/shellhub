...
func TestMongoInfraDeleted(t *testing.T) {
	...
	deleted := []string{
		filepath.Join(serverRoot, "api", "store", "mongo"),
		filepath.Join(serverRoot, "api", "store", "migrate"),
		filepath.Join(serverRoot, "api", "store", "storetest", "mongoprovider"),
		filepath.Join(serverRoot, "api", "pkg", "dbtest"),
	}
...
// MongoDB-related dependencies have been removed from server/go.mod and
// server/go.sum.
//
