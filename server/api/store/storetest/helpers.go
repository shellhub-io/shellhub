...
// uniqueHex returns n hex characters unique to this call.
//
// Fixtures used to derive their unique names and fingerprints from
// clock.Now().UnixNano(). That collides for two fixtures built inside the same
// nanosecond, and is a constant once a test mocks the clock, so it produced
// duplicate-key failures that looked like store bugs.
...
// uniqueFingerprint returns a value shaped like an SSH key fingerprint: sixteen
