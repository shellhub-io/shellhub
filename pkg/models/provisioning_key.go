...
// ProvisioningKeyMode is the per-key enrollment policy: it decides a device's initial status when the
// device enrolls with the key.
type ProvisioningKeyMode string

const (
	// ProvisioningKeyModeAutomatic accepts the device on enrollment (the classic provisioning-key behavior).
	ProvisioningKeyModeAutomatic ProvisioningKeyMode = "automatic"
	// ProvisioningKeyModeManual lands the device pending for manual review. The legacy/system key is always
	// this mode.
	ProvisioningKeyModeManual ProvisioningKeyMode = "manual"
	// ProvisioningKeyModeWebhook defers the decision to an integrator's endpoint, called at enrollment.
	ProvisioningKeyModeWebhook ProvisioningKeyMode = "webhook"
	// ProvisioningKeyModeAllowlist accepts the device when the identity it presents is in
	// AllowedIdentities, otherwise rejects it.
	ProvisioningKeyModeAllowlist ProvisioningKeyMode = "allowlist"
)

// ProvisioningKeyType discriminates a key's origin: a user-created key, or one of the two auto-managed
...
const (
	// ProvisioningKeyTypeUser is a normal user-created key.
	ProvisioningKeyTypeUser ProvisioningKeyType = "user"
	// ProvisioningKeyTypeLegacy is the tenant-only keyless enrollment source (a device presenting only a
	// tenant ID, no provisioning key). Manual mode: such devices land pending.
	ProvisioningKeyTypeLegacy ProvisioningKeyType = "legacy"
	// ProvisioningKeyTypePairing is the code-pairing enrollment source (a tenant-less agent accepted via its
	// printed code). Devices accepted through the pairing flow attribute here, not to the legacy key.
	ProvisioningKeyTypePairing ProvisioningKeyType = "pairing"
)

// Webhook tuning bounds/defaults (seconds). A stored 0 means "use the default".
const (
	// ProvisioningKeyWebhookDefaultTimeout / MaxTimeout bound the synchronous webhook request.
	ProvisioningKeyWebhookDefaultTimeout = 5
	ProvisioningKeyWebhookMaxTimeout     = 15
	// ProvisioningKeyWebhookDefaultCallbackTTL / MaxCallbackTTL bound the deferred-decision token's validity
	// (1 hour default, 24 hours max).
	ProvisioningKeyWebhookDefaultCallbackTTL = 3600
	ProvisioningKeyWebhookMaxCallbackTTL     = 86400
)
...
// ProvisioningKey is a reusable, revocable, namespace-scoped credential that decides how a device is
// enrolled. Its [ProvisioningKey.Mode] is the policy: a device enrolling with the key lands accepted,
...
// key cannot be recovered afterwards. Use [ProvisioningKey.IsValid] to verify a key can still enroll.
...
func (s *ProvisioningKey) WebhookTimeoutOrDefault() int {
	return clampOrDefault(s.WebhookTimeout, ProvisioningKeyWebhookDefaultTimeout, ProvisioningKeyWebhookMaxTimeout)
}
...
func (s *ProvisioningKey) WebhookCallbackTTLOrDefault() int {
	return clampOrDefault(s.WebhookCallbackTTL, ProvisioningKeyWebhookDefaultCallbackTTL, ProvisioningKeyWebhookMaxCallbackTTL)
}
...
func (s *ProvisioningKey) IsSystem() bool {
	return s.Type == ProvisioningKeyTypeLegacy || s.Type == ProvisioningKeyTypePairing
}
...
func (s *ProvisioningKey) IsPairing() bool {
	return s.Type == ProvisioningKeyTypePairing
}

// IsValid reports whether the provisioning key can still enroll a device: it must not be revoked, disabled,
// expired, or overused.
func (s *ProvisioningKey) IsValid() bool {
	return !s.Revoked && !s.Disabled && !s.isExpired() && !s.isOverused()
}

func (s *ProvisioningKey) isExpired() bool {
	return s.ExpiresAt != nil && clock.Now().After(*s.ExpiresAt)
}

func (s *ProvisioningKey) isOverused() bool {
	if s.UsageLimit <= 0 {
		return false
	}
...
// ProvisioningKeyConflicts holds provisioning key attributes that must be unique per tenant ID and can be used in
// queries to identify conflicts.
type ProvisioningKeyConflicts struct {
	ID   string
	Name string
}
