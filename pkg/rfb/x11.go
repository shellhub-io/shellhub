...
func (d *X11Display) CaptureScreen() ([]uint8, error) {
	...
	// Convert X11 image data to RFB format
	imageData := reply.Data

	// TODO: Don't allocate new slice every time; reuse buffer.
	rfbData := make([]uint8, len(imageData))

	// X11 typically uses BGRA format, RFB expects RGBA
	// Convert pixel format if necessary
	for i := 0; i < len(imageData); i += 4 {
		if i+3 < len(imageData) {
			// X11: BGRA -> RFB: RGBA
			rfbData[i] = imageData[i+2]   // R
			rfbData[i+1] = imageData[i+1] // G
			rfbData[i+2] = imageData[i]   // B
			rfbData[i+3] = imageData[i+3] // A
		}
	}
...
func (d *X11Display) SendKeyEvent(key uint32, down bool) error {
	// Convert RFB keysym to X11 keycode
...
// keysymToKeycode converts RFB keysym to X11 keycode
