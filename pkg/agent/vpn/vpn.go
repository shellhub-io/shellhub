...
func (s *VPN) Handler(conn net.Conn, settings *Settings) error {
	...
	go func() {
		...
		packet := make([]byte, packets.PacketMaxSize)

		for {
			log.Trace("reading from interface")

			read, err := face.Read(packet)
			if err != nil {
				log.WithError(err).Debug("error from interface reading")
			...
			log.WithFields(log.Fields{
				"size": read,
			}).Trace("read from interface")

			log.Trace("writing to connection")

			wrote, err := conn.Write(packet[:read])
			if err != nil {
				log.WithError(err).Debug("error from connection writing")
			...
			log.WithFields(log.Fields{
				...
				"destination": packets.Destination(packet),
				"protocol":    packets.Protocol(packet),
				"length":      packets.Length(packet),
				"size":        wrote,
			}).Debug("wrote to connection")
		}
	}()
	...
	go func() {
		...
		for {
			log.Trace("reading from connection")

			read, err := conn.Read(packet)
			if err != nil {
				log.WithError(err).Debug("error from connection reading")
			...
			log.WithFields(log.Fields{
				"size": read,
			}).Trace("read from connection")

			if read == 0 {
				log.Trace("zero bytes was read from the connetion")

				continue
			}

			log.Trace("writing from interface")

			wrote, err := face.Write(packet[:read])
			if err != nil {
				log.WithError(err).Debug("error from interface writing")
