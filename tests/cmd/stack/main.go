...
type stateFile struct {
	...
	ServerImageID string            `json:"server_image_id"`
}
...
func main() {
	...
	upCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if existing, err := readState(name); err == nil {
				cmd.PrintErrf("Found existing stack %q, checking...\n", projectName)

				attached, err := environment.Attach(ctx, existing.Name, existing.Files, existing.Envs)
				if err == nil {
					if existing.Edition == string(edition) {
						if runErr := attached.Running(ctx); runErr == nil {
							tagID, tagErr := environment.TagImageID(ctx, environment.ServerImageTag(edition))
							if tagErr == nil && tagID == existing.ServerImageID {
								cmd.PrintErrln("Stack is current, reusing.")

								return printExports(stdout, existing)
							}
						}
					}

					cmd.PrintErrln("Stack is stale, tearing down...")
					if err := attached.Down(ctx); err != nil {
						cmd.PrintErrf("Could not tear down stale stack: %v\n", err)
					}
				}
			...
			_, imageID, err := stack.ServerImage(ctx)
			if err != nil {
				return fmt.Errorf("reading server image: %w", err)
			}
	...
	downCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if err != nil {
				...
				orphan, attachErr := environment.Attach(ctx, projectName, []string{"../docker-compose.yml"}, nil)
				if attachErr == nil {
			...
			if err != nil {
				...
				if logErr := stack.Logs(ctx, f); logErr != nil {
